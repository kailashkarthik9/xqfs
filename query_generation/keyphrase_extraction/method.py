...
from query_generation.keyphrase_extraction.methods_embeddings import extract_candidates_embedding_for_doc, extract_doc_embedding, \
    extract_sent_candidates_embedding_for_doc


def _MMR(embdistrib, text_obj, candidates, X, idf_scores, ptagger, beta, N, use_filtered, alias_threshold):
    """
    ...
    :param idf_scores: score to scale doc_sim
    ...
    idf_scores = idf_scores.reshape(-1, 1)
    idf_scores = idf_scores.astype(float)
...
def MMRPhrase(embdistrib, text_obj, raw_text, ptagger, idf_model, beta=0.65, N=10, use_filtered=True, alias_threshold=0.8):
    """
    ...
    3)list containing for each keyphrase a list of alias (list of list of string)
    """
    candidates, embeddings, idf_scores = extract_candidates_embedding_for_doc(embdistrib, raw_text, ptagger, idf_model)

    if len(candidates) == 0:
        warnings.warn('No keyphrase extracted for this document')
        return None, None, None

    return _MMR(embdistrib, text_obj, candidates, embeddings, idf_scores, ptagger, beta, N, use_filtered, alias_threshold)
